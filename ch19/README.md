...
这个脚本定义了一个命令：`print`。该命令会将文本打印到 *STDOUT*。如果运行这个脚本，什么都不会发生。因为没有在命令行中指定文件名，因此gawk程序会从 *STDIN* 接受数据。在脚本运行时，它会一直等待来自 *STDIN* 的文本/如果你输入一行文本并按下Enter键，则gawk脚本会对这行文本执行一遍脚本。和sde编辑器一样gawk会对数据流中的每一行文本都会执行脚本。所以，你每输入一行文本，都会打印 *Hello World* 的输出。
...
替换命令在替换多行中的文本是也能正常工作，但在默认情况下它只替换每行中出现的第一处匹配文本。要想替换每行中所有的匹配文本，必须使用 **替换标志(substitution flag)** 。替换标志在替换命令字符串之后设置。
...
如你所见，该命令只应用于包含匹配模式的行。虽然是用固定的文本模式有助于过滤出特定的值，就跟上面的例子一样，但难免有所局限。sed编辑器在文本模式中引入了正则表达式（见第20章）来创建匹配效果更好的模式。
...
套用信件将通用占位文本 *LIST* 放在了人物名单的位置。要在展位文本后插入名单，只需使用读取命令即可。但这样的话，占位文本仍然会留在输出中。为此，可以用删除命令删除占位文本，其结果如下：
...
在这类系统中运行的shell脚本，如果使用 */bin/sh* 作为 shebang，则脚本会运行在 dash shell而非bash shell中。这可能会造成很多shell脚本命令执行失败。
